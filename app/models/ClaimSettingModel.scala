...
case class ClaimSetting (
    _id: BSONObjectID,
    dis: Boolean,
    sys: Option[System]
)

object ClaimSettingModel {
  ...
  implicit object LeaveSettingBSONReader extends BSONDocumentReader[ClaimSetting] {
    def read(p_doc: BSONDocument): ClaimSetting = {
      ClaimSetting(
          p_doc.getAs[BSONObjectID]("_id").get,
          p_doc.getAs[Boolean]("dis").getOrElse(false),
  ...
  implicit object ClaimSettingBSONWriter extends BSONDocumentWriter[ClaimSetting] {
    def write(p_doc: ClaimSetting): BSONDocument = {
      BSONDocument(
          "_id" -> p_doc._id,
          "dis" -> p_doc.dis,
          "sys" -> p_doc.sys
  ...
  private val col = DbConnUtility.claim_db.collection("claimsetting")
  
  val doc = ClaimSetting(
      _id = BSONObjectID.generate,
      dis = false,
      sys = None
  )
  
  private def updateSystem(p_doc:ClaimSetting) = {
  ...
  // Insert new document
  def insert(p_doc:ClaimSetting, p_eid:String="", p_request:RequestHeader=null)= {
  ...
  // Update document
  def update(p_query:BSONDocument,p_doc:ClaimSetting,p_request:RequestHeader) = {
  ...
  def find(p_query:BSONDocument) = {
    col.find(p_query).cursor[ClaimSetting](ReadPreference.primary).collect[List]()
  }
  
  // Find all documents using session
  def find(p_query:BSONDocument, p_request:RequestHeader) = {
    col.find(p_query.++(BSONDocument("sys.eid" -> p_request.session.get("entity").get, "sys.ddat"->BSONDocument("$exists"->false)))).cursor[ClaimSetting](ReadPreference.primary).collect[List]()
  }
	
  // Find one document
  // Return the first found document
  def findOne(p_query:BSONDocument) = {
    col.find(p_query).one[ClaimSetting]
  }
  
  // Find one document using session
  // Return the first found document
  def findOne(p_query:BSONDocument, p_request:RequestHeader) = {
    col.find(p_query.++(BSONDocument("sys.eid" -> p_request.session.get("entity").get, "sys.ddat"->BSONDocument("$exists"->false)))).one[ClaimSetting]
  }
