...
object LeaveModel {
  ...
  private val col = db.collection("leave")
  val doc = Leave(
  ...
  def isOnleave(p_id:String, p_dt:String, p_date: DateTime, p_request:RequestHeader) = {
    for {
      leave <- this.findOne(BSONDocument("pid"->p_id, "dt"->p_dt, "w_s"->"Approved", "fdat"->BSONDocument("$lte"->BSONDateTime(p_date.getMillis())), "tdat"->BSONDocument("$gte"->BSONDateTime(p_date.getMillis()))), p_request)
    } yield {
